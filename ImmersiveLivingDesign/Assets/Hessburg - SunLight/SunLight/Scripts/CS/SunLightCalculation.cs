...
namespace Hessburg
{
	...
	public class SunLightCalculation
	{
		...
		public void CreateLensFlare(Vector3 pos, GameObject SunLightGO)
		{
			...
			foreach (Transform t in SunLightGO.transform)
			{
			     if(t.name == "SL_LensFlare")
			     {
					...
					lensFlare=instance.GetComponent<LensFlareComponentSRP>();
			     	found=true;
			     }
			} 
		...
		public void SetSun(float latitude, float longitude, float offsetUTC, int dayOfYear, float timeInHours, float northDirection, float overcastFactor, float artisticSunAzimuth, bool overrideAzimuth)
		{
			...
			// end of New in version 1.2.1:
				...
				*/
				...
				if(lensFlare != null && lensFlare.lensFlareData != null && lensFlare.lensFlareData.elements.Length > 0)
                {
					var element = lensFlare.lensFlareData.elements[0];
					element.tint = sceneLight.color;
					element.localIntensity = Mathf.Clamp((altitude * 0.2f) + 0.22f, 0.0f, 0.5f);
					lensFlare.lensFlareData.elements[0] = element;
				}
		...
		[HideInInspector]
		//public LensFlare lensFlare; // The lensFlare will be instantiated from the prefab of the resources folder at runtime [HideInInspector]
		public LensFlareComponentSRP lensFlare; // The lensFlare will be instantiated from the prefab of the resources folder at runtime 
		[HideInInspector]
